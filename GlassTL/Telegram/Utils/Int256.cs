...
namespace BigMath
{
    ...
    public struct Int256 : IComparable<Int256>, IComparable, IEquatable<Int256>, IFormattable
    {
        ...
        public Int256(decimal value)
        {
            ...
            uint[] bits = decimal.GetBits(value).Select(x => (uint)x).ToArray();
            uint scale = (bits[3] >> 16) & 0x1F;
            if (scale > 0)
            {
                MathUtils.DivModUnsigned(bits, new[] { 10U * scale }, out uint[] quotient, out uint[] reminder);
        ...
        public string ToString(string format, IFormatProvider formatProvider = null)
        {
            ...
            if (!string.IsNullOrEmpty(format))
            {
                ...
                if ((ch == 'x') || (ch == 'X'))
                {
                    int.TryParse(format.Substring(1).Trim(), out int min);
        ...
        private string ToString(NumberFormatInfo info)
        {
            ...
            while (true)
            {
                current = DivRem(current, ten, out Int256 r);
        ...
        {
            if (!TryParse(value, style, provider, out Int256 result))
            {
        ...
        {
            if (TryConvert(conversionType, provider, asLittleEndian, out object value))
            {
                return value;
            }
        ...
        {
            return DivRem(dividend, divisor, out _);
        }
        ...
        {
            DivRem(dividend, divisor, out Int256 remainder);
            return remainder;
        }
        ...
        public static Int256 DivRem(Int256 dividend, Int256 divisor, out Int256 remainder)
        {
            ...
            MathUtils.DivModUnsigned(dividend.ToUIn32Array(), divisor.ToUIn32Array(), out uint[] quotient, out uint[] rem);
        ...
        public static explicit operator double(Int256 value)
        {
            ...
            if (!double.TryParse(value.ToString(nfi), NumberStyles.Number, nfi, out double d))
            {
                throw new OverflowException();
            }
        ...
        public static explicit operator float(Int256 value)
        {
            ...
            NumberFormatInfo nfi = CultureInfo.InvariantCulture.NumberFormat;
            if (!float.TryParse(value.ToString(nfi), NumberStyles.Number, nfi, out float f))
            {
                throw new OverflowException();
            }
