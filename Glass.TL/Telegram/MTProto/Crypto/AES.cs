...
namespace GlassTL.Telegram.MTProto.Crypto
{
    public struct AESKeyData
    {
        ...
        {
            if (!(obj is AESKeyData keyData)) return false;

            return keyData.Key.SequenceEqual(Key) && keyData.Iv.SequenceEqual(Iv);
        }
        ...
        public static bool operator ==(AESKeyData left, AESKeyData right)
        {
    ...
    public static class AES
    {
        ...
        public static AESKeyData GenerateKeyDataFromNonces(byte[] serverNonce, byte[] newNonce)
        {
            ...
            var hash1 = hash.ComputeHash(newNonce.Concat(serverNonce).ToArray());
            var hash2 = hash.ComputeHash(serverNonce.Concat(newNonce).ToArray());
            var hash3 = hash.ComputeHash(newNonce.Concat(newNonce).ToArray());

            var Key = hash1.Concat(hash2.Take(12)).ToArray();
            var IV = hash2.Skip(12).Take(8).Concat(hash3).Concat(newNonce.Take(4)).ToArray();
    ...
    public class AesEngine
    {
        ...
        public bool IsPartialBlockOkay { get; } = false;

        private uint Shift(uint r, int shift) => (r >> shift) | (r << (32 - shift));

        private uint FFmulX(uint x) => ((x & m2) << 1) ^ (((x & m1) >> 7) * m3);
        ...
        */
        ...
        private uint[,] GenerateWorkingKey(byte[] key, bool forEncryption)
        {
            ...
            if ((KC != 4) && (KC != 6) && (KC != 8)) throw new ArgumentException("Key length not 128/192/256 bits.");
